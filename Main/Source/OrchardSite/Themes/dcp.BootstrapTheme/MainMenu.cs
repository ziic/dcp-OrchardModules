        ...
        public void GetMenu(IContent menu, NavigationBuilder builder)
        {
            if (menu.As<TitlePart>().Title == "Main Menu")
            {
                var maxPosition = _contentManager
                    ...
                    .List()
                    .Select(x => Convert.ToInt32(decimal.Parse(x.MenuPosition.Split('.')[0])))
                    .Max();
                ...
                if (_orchardServices.WorkContext.CurrentUser != null)
                {
                    ...
                    builder.Add(T("Sign Out"), itemCount.ToString() + ".2",
                        item => item.Action("LogOff", "Account", new {area = "Orchard.Users", ReturnUrl = _orchardServices.WorkContext.HttpContext.Request.RawUrl}));
                ...
                {
                    builder.Add(T("Sign In"), itemCount.ToString(),
                        item => item.Action("LogOn", "Account",
                            new
                            {
                                area = "Orchard.Users",
                                ReturnUrl = (_orchardServices.WorkContext.HttpContext.Request.QueryString["ReturnUrl"] ?? _orchardServices.WorkContext.HttpContext.Request.RawUrl)
                            }));
                }
            }
        }
